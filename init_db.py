    ...
    with conn.cursor() as cur:
        ...
        for utilisateur in root_u.findall("utilisateur"):
            ...
            for objet in achats.findall("objet"):
                ...
                if id_objet is None:
                    print(f"  Objet non trouvé : '{objet.text}' (normalisé: '{nom_normalise}')")
                    continue

                cur.execute("SELECT Prix FROM ObjetCosmetique WHERE Id = %s", (id_objet,))
                res_prix = cur.fetchone()
                prix = res_prix['prix']

                cur.execute("""
                    INSERT INTO Transaction (Date, Montant, Type, IdObjetCosmetique, IdUtilisateur)
                    VALUES (CURRENT_DATE, %s, 'depense', %s, %s)
                """, (-prix, id_objet, uid))
